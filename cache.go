...
func Expired[T Storable](item Item[T]) bool {
...
func (c *cache[T]) Set(k string, t T, d time.Duration) {
	// "Inlining" of set
	var e int64
...
// expiration.
func (c *cache[T]) SetDefault(k string, t T) {
	c.Set(k, t, DefaultExpiration)
}

// Add an item to the cache only if an item doesn't already exist for the given
// key, or if the existing item has expired. Returns an error otherwise.
func (c *cache[T]) Add(k string, t T, d time.Duration) error {
	c.mu.Lock()
	_, found := c.get(k)
	if found {
		c.mu.Unlock()
...
func (c *cache[T]) Replace(k string, t T, d time.Duration) error {
	c.mu.Lock()
	_, found := c.get(k)
	if !found {
		c.mu.Unlock()
...
func Run[T Storable](j *janitor, c *cache[T]) {
...
func stopJanitor[T Storable](c *Cache[T]) {
	c.janitor.stop <- true
}

func runJanitor[T Storable](c *cache[T], ci time.Duration) {
	j := &janitor{
		Interval: ci,
		stop:     make(chan bool),
	}
...
func newCache[T Storable](de time.Duration, m map[string]Item[T]) *cache[T] {
	if de == 0 {
		de = -1
	}
	c := &cache[T]{
...
func newCacheWithJanitor[T Storable](de time.Duration, ci time.Duration, m map[string]Item[T]) *Cache[T] {
	c := newCache(de, m)
...
func New[T Storable](defaultExpiration, cleanupInterval time.Duration) *Cache[T] {
...
func NewFrom[T Storable](defaultExpiration, cleanupInterval time.Duration, items map[string]Item[T]) *Cache[T] {
	return newCacheWithJanitor(defaultExpiration, cleanupInterval, items)
}
